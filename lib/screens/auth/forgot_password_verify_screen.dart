...
class _ForgotPasswordVerifyScreenState extends State<ForgotPasswordVerifyScreen> {
  ...
  Future<void> _verifyRecoveryOtp() async {
    ...
    if (token.length < 6) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Harap masukkan 6-digit kode OTP pemulihan lengkap'),
    ...
    final success = await authProvider.verifyRecoveryOtp(widget.email, token);

    if (!mounted) return;

    if (success) {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (_) => const ForgotPasswordResetScreen()),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(authProvider.errorMessage ?? 'Kode OTP pemulihan tidak valid atau kadaluarsa.'),
          backgroundColor: AppColors.danger,
        ),
      );
    }
  ...
  Future<void> _handleResendResetOtp() async {
    final authProvider = context.read<AuthProvider>();
    final success = await authProvider.sendResetPasswordEmail(widget.email);
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Kode OTP pemulihan baru telah dikirimkan ke email ${widget.email}'),
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Masukkan 6-digit kode pemulihan yang telah dikirimkan ke email ${widget.email}',
                    ...
                    child: Column(
                      children: [
                        Pinput(
                          ...
                          onCompleted: (pin) => _verifyRecoveryOtp(),
                        ),
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            GestureDetector(
                              onTap: _canResend ? _handleResendResetOtp : null,
                              child: Text(
                        ...
                        Container(
                          ...
                          child: const Row(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  'Tips: Cek folder Spam di email Anda jika kode reset belum muncul di Inbox Utama.',
                                  style: TextStyle(
                                    fontSize: 11,
                        ...
                        const SizedBox(height: 24),
                        ElevatedButton(
                          onPressed: authProvider.isLoading ? null : _verifyRecoveryOtp,
